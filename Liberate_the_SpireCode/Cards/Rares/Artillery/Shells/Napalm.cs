...
public class Napalm() : Liberate_the_SpireCard(1, CardType.Attack, CardRarity.Token, TargetType.AllEnemies)
{
    protected override IEnumerable<DynamicVar> CanonicalVars => [new DamageVar(3, ValueProp.Move), new PowerVar<BurnPower>(5)];
    ...
    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay play)
    {
        ...
        ArgumentNullException.ThrowIfNull(shell.CombatState, "CombatState - shell null");
        
        IReadOnlyList<BurnPower> burnPowerList = await PowerCmd.Apply<BurnPower>(choiceContext,
            shell.CombatState.HittableEnemies, shell.DynamicVars.Power<BurnPower>().BaseValue, shell.Owner.Creature, shell);
